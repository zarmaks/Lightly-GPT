...
def find_duplicate_images(threshold_str="5"):
    ...
    try:
        ...
        # Display results
        if not duplicates:
            st.info(f"🎉 No duplicate images found with similarity threshold {threshold}/10.")
        ...
        for i, dup in enumerate(duplicates):
            with st.expander(f"Duplicate Pair {i+1}: {dup['similarity']:.1f}% Similar", expanded=True):
                col1, col2 = st.columns(2)
                  # Display first image
                with col1:
                    st.write(f"**Image {dup['image1']['index']}**: {dup['image1']['filename']}")
                    try:
                        dup['image1']['file'].seek(0)
                        img1 = Image.open(dup['image1']['file']).convert('RGB')
                        # Resize for better display
                        img1_resized = resize_image(img1, max_size=400)
                        st.image(img1_resized, caption=f"Index: {dup['image1']['index']}", use_column_width=True)
                    except Exception as e:
                        st.error(f"Could not display image: {str(e)}")
                
                # Display second image
                with col2:
                    st.write(f"**Image {dup['image2']['index']}**: {dup['image2']['filename']}")
                    try:
                        dup['image2']['file'].seek(0)
                        img2 = Image.open(dup['image2']['file']).convert('RGB')
                        # Resize for better display
                        img2_resized = resize_image(img2, max_size=400)
                        st.image(img2_resized, caption=f"Index: {dup['image2']['index']}", use_column_width=True)
                    except Exception as e:
                        st.error(f"Could not display image: {str(e)}")
                ...
                with col_actions:
                    st.write("**Actions:**")
                    if st.button(f"Remove Image {dup['image1']['index']}", key=f"remove_{i}_1"):
                        st.info(f"Would remove image {dup['image1']['index']}: {dup['image1']['filename']}")
                        st.write("*(Feature can be implemented to actually remove from session state)*")
                    
                    if st.button(f"Remove Image {dup['image2']['index']}", key=f"remove_{i}_2"):
                        st.info(f"Would remove image {dup['image2']['index']}: {dup['image2']['filename']}")
                        st.write("*(Feature can be implemented to actually remove from session state)*")
