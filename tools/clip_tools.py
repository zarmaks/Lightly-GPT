...
def clip_image_search_tool(query, threshold=0.75):
    """
    Search for images matching a text description using CLIP embeddings

    Args:
        query: Text description to search for
        threshold: Maximum distance threshold for similarity (default 0.75, lower = more similar)
    ...
    try:
        ...
        # Search for similar images in ChromaDB (get all images to filter by threshold)
        results = st.session_state.collection.query(
            query_embeddings=[text_embedding.tolist()],
            n_results=len(st.session_state.uploaded_images),  # Get all images
        )
        ...
        for i, idx in enumerate(filtered_indices):
            if 0 <= idx < len(st.session_state.uploaded_images):
                ...
                filename = img.name
                captions.append(
                    f"Image {idx}: {filename} (distance: {filtered_distances[i]:.2f})"
                )
                response += (
                    f"Image {idx}: {filename} (distance: {filtered_distances[i]:.2f})\n"
                )
