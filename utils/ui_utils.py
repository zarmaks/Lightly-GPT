...
def display_image_grid(images, num_columns=4, captions=None):
    ...
    for i, img_file in enumerate(images):
        ...
        with columns[col_idx]:
            try:
                img_file.seek(0)
                caption = captions[i] if captions and i < len(captions) else img_file.name
...
def styled_container(content_function, border_color="#eeeeee", padding="1em", border_radius="0.5em"):
    """
...
def create_progress_tracker(steps, current_step):
    ...
    for i, step in enumerate(steps):
        status = "completed" if i < current_step else "current" if i == current_step else "upcoming"
        color = "#4CAF50" if status == "completed" else "#2196F3" if status == "current" else "#ddd"
...
def create_image_card(img_file, title=None, metadata=None):
    ...
    with st.container():
        if title:
            st.markdown(f"<h3 style='margin-top: 0.5em;'>{title}</h3>", unsafe_allow_html=True)
        else:
            st.markdown(f"<h3 style='margin-top: 0.5em;'>{img_file.name}</h3>", unsafe_allow_html=True)
