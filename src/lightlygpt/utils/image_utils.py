...
def create_thumbnail(img_file, size=(200, 200)):
    ...
    img.thumbnail(
        size, Image.LANCZOS if hasattr(Image, "LANCZOS") else Image.Resampling.LANCZOS
    )
    return img
...
def get_image_foreground_mask(img):
    ...
    try:
        ...
        # Apply GrabCut
        cv2.grabCut(
            img_cv, mask, rectangle, bgd_model, fgd_model, 5, cv2.GC_INIT_WITH_RECT
        )
...
def show_dependency_warnings():
    ...
    if not CV2_AVAILABLE:
        st.warning(
            "OpenCV (cv2) not found. Some image processing features will be disabled."
        )
    if not IMAGEHASH_AVAILABLE:
